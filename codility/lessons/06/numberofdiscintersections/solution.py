...
def solution(radii: List[int]):
    ...
    for disc_number, radius in enumerate(radii):
        discs_on_points = populate_points_with_disc_numbers(radius, disc_number, discs_on_points)
...
def populate_points_with_disc_numbers(radius: int, disc_number: int, discs_on_points: List[List[int]]) -> List[List[int]]:
